...
export const startSshAgent = async (
	...
	try {
		...
		if (status.running) {
			log.info("SSHield agent is already running")
		...
		const spinner = createSpinner("Starting SSHield agent...")
		spinner.start()

		// Start the agent
		const { agentPid, sshAuthSock } = await startAgent()

		spinner.succeed("SSHield agent started successfully")
		...
		log.error(`Failed to start SSHield agent: ${error.message}`)
		throw error
	}
}
...
export const stopSshAgent = async (
	...
	try {
		...
		if (!status.running) {
			log.info("SSHield agent is not running")
			return
		}
		...
		if (!options.json) {
			const confirmed = await prompt.confirm(
				"Are you sure you want to stop the SSHield agent? All loaded keys will be unloaded.",
				false
			)
		...
		const spinner = createSpinner("Stopping SSHield agent...")
		spinner.start()

		// Stop the agent
		await stopAgent()

		spinner.succeed("SSHield agent stopped successfully")
	} catch (error: any) {
		log.error(`Failed to stop SSHield agent: ${error.message}`)
		throw error
	}
}
...
export const generateAgentStartupScript = async (
	...
	try {
		...
		// Default output path
		const outputPath =
			options.outputPath || `${process.env.HOME}/.sshield-agent.sh`

		// Create the script content
		const scriptContent = `#!/bin/bash
  # SSH Agent startup script generated by sshield
  # Project: ${projectId}
  ...
  SSH_MANAGER_DIR="${process.env.HOME}/.sshield"
  
  # Check if the sshield CLI is available
  if command -v sshield &> /dev/null; then
    echo "Adding keys from project ${projectId}..."
    sshield agent add-project-keys --project ${projectId} ${
			options.lifetime ? `--lifetime ${options.lifetime}` : ""
		}
  else
    echo "sshield CLI not found. You need to install it to use this script."
  fi
...
export const exportAgentEnvironment = async (
	...
	try {
		...
		const envContent = `# SSH Agent environment variables exported by sshield
  # Generated: ${new Date().toISOString()}
